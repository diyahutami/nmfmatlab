function [W,H,objhistory,iter,elapsed] = nmfscnew( V, rdim, sW, sH, fname, showflag, stopconv, tol,timelimit, maxiter )
...
for iter=1:maxiter,
    ...
    if showflag & (rem(iter,5)==0),
        ...
        % added for now
        % figure(100); 
        % imstiled(reshape(W,15,15,25),[],'gray');
        drawnow;
    end
    ...
    if ~isempty(sH),
        ...
        while 1,
            ...
            Hnew = H - stepsizeH*dH;
            for i=1:rdim, 
                Hnew(i,:) = (projfunc(Hnew(i,:)',L1s,1,1))'; 
            end
